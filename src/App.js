...
function App() {
  ...
  // Carregar os 'To dos" ao carregar a pagina

  useEffect(() => {

    const loadData = async() => {

      // segura e a pagina esperar carregar os dados
      setLoading(true)

      const res = await fetch(API + "/todos").then((res) => res.json()).then((data) => data).catch((err) => console.log(err));

      // "solta" a pagina, encerra o carregamento
      setLoading(false)
  ...
  const handleSubmit = async (e) => {
    ...
    //atualiznaod dinamicamente a pagina ao inserir uma nova tarefa
  ...
  const handleEdit = async (todo) => {
    ...
    //inverso do status atual da tarefa
    todo.done = !todo.done
  ...
  //Evitar mostrar que há 0 tarefas, porque a pagina ainda não terminou de carregar.
  if(loading) {
      ...
      <div className="form-todo">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className='form-control'>
            <label htmlFor='title'>Qual é a tarefa?</label>
